...
for i in range(len(df)):
    ...
    previous_context = df["previous_context"].iloc[i] if "previous_context" in df.columns else None
    ...
    if previous_context and pd.notna(previous_context):
        user_message = f"Previous context: {previous_context}\n\nText to classify: {text}"
    else:
...
df['Mixtral-8x22B-Instruct-v0.1'] = targets

# Save results
output_filename = 'Mixtral-8x22B-Instruct-v0.1-2.csv'
